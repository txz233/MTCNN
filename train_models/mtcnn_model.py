...
def landmark_ohem(landmark_pred,landmark_target,label):
    ...
    zeros_index = tf.zeros_like(label, dtype=tf.float32)
    ones_index = tf.ones_like(label, dtype=tf.float32)
    valid_ind = tf.where(tf.equal(label,-2),ones_index,zeros_index)
    square_error =  tf.square(landmark_pred-landmark_target)
    square_error = tf.reduce_sum(square_error,axis=1)
    num_valid = tf.reduce_sum(valid_ind)
    keep_num=tf.cast(num_valid,dtype = tf.int32)
    square_error =square_error*valid_ind
    _,k_index = tf.nn.top_k(square_error,k=keep_num)
    square_error = tf.gather(square_error,k_index)
    return tf.reduce_mean(square_error)
...
def O_Net(inputs,label=None,bbox_target=None,landmark_target=None,training=True):
    ...
    with slim.arg_scope(
        ...
        net = slim.max_pool2d(net, kernel_size=[3, 3], stride=2, scope="pool1", padding='SAME')
        print(net.get_shape())
        net = slim.conv2d(inputs,num_outputs=64,kernel_size=[3,3],stride=1,scope='conv2')
        print(net.get_shape())
        net = slim.max_pool2d(net,kernel_size=[3,3],stride=2,scope="pool2")
        print(net.get_shape())
        net = slim.conv2d(inputs,num_outputs=64,kernel_size=[3,3],stride=1,scope='conv3')
        print(net.get_shape())
